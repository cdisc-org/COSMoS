...
def get_sdtms(client, cosmos_api_version, all_sdtm_list, count=100000):
    ...
    for sdtm in all_sdtm_list:
        ...
        datasetSpecialization = client.get_sdtm_latest_sdtm_datasetspecialization(
            cosmos_api_version, datasetSpecializationId
        )
...
def get_sdtm_variable_data(sdtm):
    ...
    for v in variables:
        ...
        variable_list.append([
            v_order, sdtm_variable, dec_id, nsv_flag, codelist, codelist_submission_value,
            subset_codelist, value_list, assigned_term, assigned_value, role, subject,
            linking_phrase, predicate_term, object, data_type, length, format,
            significant_digits, mandatory_variable, mandatory_value, origin_type,
...
def update_readme(workbook, sheetname, source, date, count):
    ...
    if source.lower() == "yaml":
        intro_text = (
            f"This spreadsheet contains CDISC SDTM Dataset Specializations loaded from {count} YAML files "
            f"to be released on {date}.\n"
            f"The spreadsheet contains {count} unique CDISC SDTM Dataset Specializations.\n"
        ...
        intro_text = (
            f"This spreadsheet contains the latest versions of CDISC SDTM Dataset Specializations "
            f"in the CDISC Library as of {date}.\n"
            f"There are currently {count} unique CDISC SDTM Dataset Specializations in the CDISC Library.\n"
            "The image on the right shows the relation between Biomedical Concepts and SDTM Dataset Specializations.\n"
            "Only a few attributes are shown in the image."
        )
    ...
    for row_num, row_data in enumerate(df.values, 2):
        for col_num, col_data in enumerate(row_data, 1):
            if headers_bc[col_num - 1] in ['bc_id', 'dec_id', 'codelist']:  # add hyperlink to bc_id, dec_id, codelist
                ...
                ws.cell(row=row_num, column=col_num).value = col_data
            if headers_bc[col_num - 1] in ['short_name', 'value_list', 'linking_phrase']:
...
def set_cmd_line_args():
    ...
    parser.add_argument(
        "-o",
        "--excel_file",
        default="cdisc_sdtm_dataset_specializations_latest.xlsx",
        help="Excel file to write the SDTM Dataset Specializations to",
        dest="excel_file"
    )
...
def main():
    ...
    HEADERS_SDTM_CORE = [
        "package_date", "bc_id", "sdtmig_start_version", "sdtmig_end_version",
        "domain", "vlm_source", "vlm_group_id", "short_name"
    ]
    HEADERS_SDTM_VARIABLE = [
        "sdtm_variable", "dec_id", "nsv_flag", "codelist", "codelist_submission_value", "subset_codelist",
