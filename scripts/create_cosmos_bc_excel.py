...
def create_bc_hierarchy(df, headers_bc_hierarchy):
    ...
    # Add a column that counts the number of non-empty dec_id per bc_id,
    # excluding rows where all dec_id values are empty
    df_new = df
...
def write_biomedical_concepts_to_excel(workbook, sheetname, df):
    ...
    for row_num, row_data in enumerate(df.values, 2):
        for col_num, col_data in enumerate(row_data, 1):
            # add hyperlink to ncit_code, parent_bc_id, ncit_dec_code
            if headers_bc[col_num - 1] in [
                'ncit_code', 'parent_bc_id', 'ncit_dec_code'
            ]:
                try:
...
def update_readme(workbook, sheetname, source, date, count):
    ...
    if source.lower() == "yaml":
        intro_text = (
            f"This spreadsheet contains CDISC Biomedical Concepts loaded from {count} YAML files "
            f"to be released on {date}.\n"
            f"The spreadsheet contains {count} unique CDISC Biomedical Concepts.\n"
        ...
        intro_text = (
            f"This spreadsheet contains the latest versions of CDISC Biomedical Concepts in the CDISC Library "
            f"as of {date}.\n"
            f"There are currently {count} unique CDISC Biomedical Concepts in the CDISC Library.\n"
...
def set_cmd_line_args():
    ...
    parser.add_argument(
        "-o",
        "--excel_file",
        default="cdisc_biomedical_concepts_latest.xlsx",
        help="Excel file to write the Biomedical Concepts to",
        dest="excel_file"
    )
...
def main():
    ...
    HEADERS_BC_HIERARCHY = [
        'bc_short_name', 'bc_id', 'bc_short_name_id', 'parent_bc_id',
